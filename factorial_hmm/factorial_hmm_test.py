...
class BasicFactorialHMM(FactorialHMMDiscreteObserved):
    ...
    def SetTransitionMatricesTensor(self):
        ...
        for n in range(self.n_steps - 1):
            ...
            self.SetTransitionSubmatrix(n, self.I.trit,
                                        [[1 - p, p * (1 - p2), 1 / 3],
                                         [p * p2, 1 - p, 1 / 3],
                                         [p * (1 - p2), p * p2, 1 / 3]])
...
class GaussianFactorialHMM(FactorialHMMGeneralObserved):
    ...
    def __init__(self, params, n_steps, calculate_on_init=True):
        ...
        # Draw means: one mean for each combination of hidden state
        # and observation. Assume variance=0.1
        self.mus = m + np.random.RandomState().rand(
            *(self.hidden_indices.field_sizes + [len(self.observed_indices)]))

    def SetTransitionMatricesTensor(self):
        for n in range(self.n_steps - 1):
            self.SetTransitionSubmatrix(n, self.I.bit_a,
                                        [[0.2, 0.6], [0.8, 0.4]])
            self.SetTransitionSubmatrix(n, self.I.bit_b,
                                        [[0.3, 0.5], [0.7, 0.5]])
    ...
    def GetObservedGivenHidden(self, observed_state, n_step):
        # Assume, for this example, that the two parts of the observation
        # are independent
        a = np.ones(self.hidden_indices.field_sizes)
        for st in self.all_hidden_states:
            a[tuple(st)] = np.prod(
                scipy.stats.norm(loc=self.mus[list(st) + [Ellipsis]],
                                 scale=0.1).pdf(observed_state))
        return a

    def DrawObservedGivenHidden(self, hidden_state, n_step, random_state):
        return scipy.stats.norm(loc=self.mus[list(hidden_state) + [Ellipsis]],
                                scale=0.1).rvs(size=len(self.observed_indices),
