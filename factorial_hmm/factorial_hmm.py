...
class FactorialHMM(object):
    def __init__(self, params, n_steps, calculate_on_init=True):
        ...
        self.all_hidden_states = np.array(list(itertools.product(
            *[range(size) for size in self.hidden_indices.field_sizes])))

        self.transition_matrices_tensor = [
            [np.ones((size, size)) for size in self.hidden_indices.field_sizes]
            for n in range(n_steps - 1)]

        # Preparation for multiply
        self.idx_in = np.arange(self.n_hidden_states, dtype=int).tolist()
        self.idx_out = np.zeros((self.n_hidden_states, self.n_hidden_states),
                                dtype=int)
        ...
        self.initial_hidden_state_tensor = [np.ones(size) for size in
                                            self.hidden_indices.field_sizes]
    ...
    def GetTransitionMatrix(self, n_step):
        return functools.reduce(np.kron,
                                self.transition_matrices_tensor[n_step])
    ...
    def InitialHiddenStateConditional(self, initial_observed_state):
        initial_hidden_state = functools.reduce(np.kron,
                                                self.initial_hidden_state_tensor).reshape(
            self.hidden_indices.field_sizes)
        ...
        initial_hidden_state *= self.GetObservedGivenHidden(
            initial_observed_state, 0)
    ...
    def Simulate(self, random_seed=0):
        ...
        X = np.zeros((self.n_observed_states, self.n_steps),
                     dtype=self.observed_type)
        for n_step in range(self.n_steps):
            # probs = self.obs_given_hidden[[Ellipsis] + list(Z[:, n_step])].ravel()
            # X[:, n_step] = self.all_observed_states[draw(probs)]
            X[:, n_step] = self.DrawObservedGivenHidden(Z[:, n_step], n_step,
                                                        rng)
    ...
    def Forward(self, observed_states):
        ...
        # Alphas
        alphas[..., 0] = self.InitialHiddenStateConditional(
            observed_states[..., 0])
        ...
        for n_step in range(1, n_steps):
            # alphas[..., n_step] =
            # self.obs_given_hidden[list(observed_states[..., n_step]) + [Ellipsis]]
            alphas[..., n_step] = self.GetObservedGivenHidden(
                observed_states[..., n_step], n_step)
            alphas[..., n_step] *= self.MultiplyTransitionMatrixVector(
                n_step - 1, alphas[..., n_step - 1])
    ...
    def EStep(self, observed_states):
        ...
        # Forward
        alphas, scaling_constants, log_likelihood = self.Forward(
            observed_states)
        ...
        for n_step in range(n_steps - 2, -1, -1):
            # vec = self.obs_given_hidden[list(observed_states[:, n_step+1]) +
            # [Ellipsis]]
            vec = self.GetObservedGivenHidden(observed_states[..., n_step + 1],
                                              n_step + 1)
            betas[..., n_step] = \
                self.MultiplyTransitionMatrixVector(n_step,
                                                    vec *
                                                    betas[
                                                        ..., n_step + 1],
                                                    transpose=True)
    ...
    def CalculateXis(self, observed_states, alphas, betas, scaling_constants):
        ...
        for n_step in range(1, n_steps):
            xis[..., n_step - 1] = (
                    alphas[..., n_step - 1].ravel()[:, np.newaxis] \
                    # * self.obs_given_hidden[list(observed_states[:, n_step])
                    # + [Ellipsis]].ravel()[np.newaxis, :] \
                    * self.GetObservedGivenHidden(
                observed_states[..., n_step], n_step).ravel()[np.newaxis, :] \
    ...
    def CollapseGammas(self, gammas, fields):
        ...
        return gammas.sum(tuple(set(range(self.n_hidden_states + 1)) - set(
            fields + [self.n_hidden_states])))
    ...
    def CollapseXis(self, xis, fields):
        ...
        return xis.sum(tuple(set(range(2 * self.n_hidden_states + 1)) - \
                             set(fields + [x + self.n_hidden_states for x in
                                           fields] + [
                                     2 * self.n_hidden_states])))

    def CalculateAndCollapseXis(self, field, observed_states, alphas, betas,
                                scaling_constants):
        if len(observed_states.shape) == 1:
            observed_states = observed_states.reshape((1, -1))
        ...
        project = np.equal.outer(
            ...
            np.arange(field_size)).reshape(
            self.hidden_indices.field_sizes + [field_size])
        ...
        for n_step in range(1, n_steps):
            # b = self.obs_given_hidden[list(observed_states[:, n_step]) +
            # [Ellipsis]] * betas[..., n_step]
            ...
            mbp = np.stack([self.MultiplyTransitionMatrixVector(n_step - 1,
                                                                bp[..., i],
                                                                transpose=True)
                            for i in range(field_size)], axis=-1)
    ...
    def GetXiRow(self, hidden_state_row, n_step, observed_states, alphas, betas,
                 scaling_constants):
        ...
        bb = betas[..., n_step + 1] \
             * self.GetObservedGivenHidden(observed_states[..., n_step + 1],
                                           n_step + 1) \
    ...
    def ViterbiSlower(self, observed_states):
        ...
        back_pointers = np.ones((self.n_hidden_state_space, n_steps - 1),
                                dtype=int)
        lls = np.ones((self.n_hidden_state_space, n_steps), dtype=float)

        ll = np.log(
            self.InitialHiddenStateConditional(observed_states[:, 0])).reshape(
            (self.n_hidden_state_space))
        lls[:, 0] = ll
        ...
        for n_step in range(1, n_steps):
            ...
            # ll = np.log(self.obs_given_hidden[list(observed_states[:, n_step])
            # + [Ellipsis]]).reshape((self.n_hidden_state_space))
            ll = np.log(
                self.GetObservedGivenHidden(observed_states[..., n_step],
                                            n_step)).reshape(
                (self.n_hidden_state_space))
            ll += M[np.arange(self.n_hidden_state_space), back_pointers[:,
                                                          n_step - 1]]
            lls[:, n_step] = ll
    ...
    def Viterbi(self, observed_states):
        ...
        mgrid_prepared = list(
            np.mgrid[[range(s) for s in self.hidden_indices.field_sizes]])

        # back_pointers = np.ones((self.n_hidden_state_space, n_steps-1), dtype=int)
        back_pointers = np.ones(
            self.hidden_indices.field_sizes + [self.n_hidden_states] + [
                n_steps - 1], dtype=int)
        lls = np.zeros(self.hidden_indices.field_sizes + [n_steps], dtype=float)

        lls[..., 0] = np.log(
            self.InitialHiddenStateConditional(observed_states[..., 0]))
        ll = lls[..., 0]
        ...
        for n_step in range(1, n_steps):
            ...
            for i in range(self.n_hidden_states):
                submatrix = np.nan_to_num(np.log(
                    self.transition_matrices_tensor[n_step - 1][i][:, :]))
            ...
            back_pointers[..., self.n_hidden_states - 1, n_step - 1] = argmaxes[
        ...
        most_likely[..., n_steps - 1] = np.unravel_index(np.argmax(ll.ravel()),
                                                         self.hidden_indices.field_sizes)
        last = most_likely[..., n_steps - 1]
        ...
        for n_step in range(n_steps - 1, 0, -1):
            ...
            most_likely[..., n_step - 1] = back_pointers[
                last.tolist() + [slice(None), n_step - 1]]
            last = most_likely[..., n_step - 1]
    ...
    def CalculateJointLogLikelihood(self, observed_states, hidden_states):
        ...
        logp += np.log(
            self.InitialHiddenStateConditional(observed_states[..., 0])[
                tuple(hidden_states[..., 0])])

        for n_step in range(1, n_steps):
            # p(z_n | z_{n-1})
            col = self.GetTransitionMatrixColumn(n_step - 1,
                                                 hidden_states[..., n_step - 1])
            logp += np.log(col[np.ravel_multi_index(hidden_states[..., n_step],
                                                    self.hidden_indices.field_sizes)])
            ...
            logp += np.log(
                self.GetObservedGivenHidden(observed_states[..., n_step],
                                            n_step)[
                    tuple(hidden_states[..., n_step])])

        return logp

    def DrawFromPosterior(self, observed_states, alphas, betas,
                          scaling_constants, start=0, end=None,
                          random_seed=None):
        if end == None:
            end = self.n_steps
        ...
        Z[:, 0] = self.all_hidden_states[
            draw(initial_hidden_state_posterior.ravel())]

        # Continue the hidden states
        for n_step in range(start + 1, end):
            xi_row = self.GetXiRow(Z[:, n_step - 1 - start], n_step - 1,
                                   observed_states, alphas, betas,
                                   scaling_constants)
            xi_row /= xi_row.sum()
    ...
    def DescaleAlphasBetas(self, alphas, betas, scaling_constants):
        ...
        acc_scaling_constants_from_back = np.multiply.accumulate(
            scaling_constants[::-1])[::-1]
        acc_scaling_constants_from_back[:-1] = acc_scaling_constants_from_back[
                                               1:]
        acc_scaling_constants_from_back[-1] = 1.0

        descaled_alphas = alphas * acc_scaling_constants[
...
class FactorialHMMDiscreteObserved(FactorialHMM):
    def __init__(self, params, n_steps, calculate_on_init=True):
        ...
        self.n_observed_state_space = functools.reduce(operator.mul,
    ...
    def DrawObservedGivenHidden(self, hidden_state, n_step, random_state):
        ...
        probs = self.obs_given_hidden[
            [n_step] + list(hidden_state) + [Ellipsis]].ravel()
...
class FullDiscreteFactorialHMM(FactorialHMMDiscreteObserved):
    def __init__(self, params, n_steps, calculate_on_init=False):
        ...
        self.hidden_indices = self.I = Indices(
            [['z{}'.format(i), params['hidden_alphabet_size']] for i in
             range(params['n_hidden_chains'])])
        self.observed_indices = self.J = Indices(
            [['x{}'.format(i), params['observed_alphabet_size']] for i in
             range(params['n_observed_chains'])])
    ...
    def SetTransitionMatricesTensor(self):
        for n in range(self.n_steps - 1):
            for field in range(self.n_hidden_states):
                self.SetTransitionSubmatrix(n, field,
                                            self.params['transition_matrices'][
                                            field, :, :])

    def SetInitialHiddenState(self):
        for field in range(self.n_hidden_states):
            self.SetInitialHiddenSubstate(field,
                                          self.params['initial_hidden_state'][
                                          field, :])

    def SetObservedGivenHidden(self):
        for n_step in range(self.n_steps):
            self.SetObservedGivenHiddenSubmatrix(n_step, self.params[
                'obs_given_hidden'])
    ...
    def EM(self, observed_states, n_iterations=1e8, likelihood_precision=1e-10,
           verbose=False, print_every=1, random_seed=None):
        ...
        G = random_state.random_sample(
            self.hidden_indices.field_sizes + self.observed_indices.field_sizes)
        G /= G.sum(axis=tuple(np.arange(self.n_hidden_states,
                                        self.n_hidden_states +
                                        self.n_observed_states)))[
            (Ellipsis,) + (np.newaxis,) * self.n_observed_states]
